...
trial_data <- trial_data %>%
  ...
  select(-stimulus, -looking_time, -MovieStart, -MovieEnd, -TrialTime, -total_fixation) 

```

#Merging Tobii with participants and order and cleaning up file
```{r}
joined_dataset_clean <- long_dataset_clean %>%
  left_join(participants, by ="subid") %>% # get orders for each participant
  left_join(orders) %>% # gets trial orders
...
### data checking
```{r}
joined_dataset_clean %>% filter(looking_time == 0 & trial_error == "noerror") %>% View()
```
